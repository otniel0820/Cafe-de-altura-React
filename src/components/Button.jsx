...
const handleClick= ()=>{
  ...
  const borrar = 'flex py-[0.75em] pl-[1.5em] pr-[1.9em] items-start gap-[0.5em] rounded bg-[red] text-[0.9em] not-italic font-semibold text-[#fff]'
 
  return (
    <div>
      
      <button className={`${style==='nav'?nav:style==='descubrirOrigenes'?descubrirOrigenes:style==='comprarCafe'?comprarCafe:style==='añadir'?añadir:style==='footer'?footer:style==='btnTotal'?checkout: style=== 'seguirComprando'? seguirComprando: style=== 'masMenos'?addAndSub:style==='agotado'?agotado:borrar}`} onClick={handleClick}><img src={icon==='tel'?phone:icon==='mail'?mailWhite: icon=== 'add'? add: icon==='sub'? sub: null} alt=""/>{text}</button>
    </div>
  )
}
